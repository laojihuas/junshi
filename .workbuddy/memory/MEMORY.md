...
- **检索（vB，2026-08-06 话术库版）**：语义词(semanticKws×2) + 规则词/原句(×1.5) → kb_blocks_recall RPC → 命中块原文直入 system（同文档≤2块，总≤3块[参考条数 v59 5→3]，**候选池 12 块** gem 精排[v76 739 块小库 24→12]）；**整句压缩(v12) 已移除**——话术库无"问题描述"语域，LLM 整句短语命中 0-37 块 vs 原句 bigram 270-596 块，纯浪费一次 LLM 调用；标题兜底 browseBlocksByTitle（话术块无块标题，靠文档名）
...
- **套路（v7→v18）**：独立惯例检索通道（resolveStrategySearchKws 按目标/阶段 5 词 → recallBlocks 捞 5 条，≥2 条有实义 + STRATEGY_HINT_RE 特征词命中才触发 LLM 提炼）；extractStrategy 提炼 2-6 步存 memory_card.strategy。**v17 灵魂保留**：输出新增 essence（核心原理/节奏/分寸 ≤60字）+ 每步强制带原文例句（引号原样话术禁官方腔）+ 时机提示 + maxTokens 400→800；buildSystemContent 注入 essence 供临场发挥、例句按语境微调。**v18 里程碑可选融合**：extractStrategy 新增 memCard 参数，prompt 末尾追加可选化段落（pending milestones + MILESTONE_TIPS 话术方向 + 5 条铁律：套路节奏优先/不生硬就不带/宁缺毋滥/已完成不重复/不为塞目标多走一步），只有 pending 不空才注入；调用点（494-513）同步传 memoryCard。**门槛低（无质量评估）**，简单思路也可能上套路。`/` 开头输入清除；轮次上限 `Math.max(steps.length+1,3)` 自动终止（v16）；对方抛更有趣话题优先跟随、套路自然搁置；**状态感知配额已失效（2026-08-06 删库后仅剩话术，hs 吃满参考配额）**
...
- **v73 迷男精髓战术卡组（version 73/74）**：替换旧"语气态度/兴趣信号/攻击检测"三块 → TACTIC_CARDS（23 卡：防守8/进攻6/救场4 + 阶段卡吸引/舒适/诱惑，来源桌面《迷男精髓.md》）；**按类注入**（每轮=全局原则+阶段卡+命中类别卡组 ≈1000 字，全量 3000 字太贵）；判定 resolveTacticCategory 纯规则零 LLM——回合数=user 消息数（≤3/≤8/9+），**DEFENSE 强/弱分级**（弱信号需整条 ≤4 字防"没意思"误伤），SELF_NEED（self 最近暴露需求）或邀约被拒→救场，默认进攻；**字数整条 ≤20 字**（用户定稿）；**后台 system_prompt 705→104 字**（身份+兜底，8 应对表被 23 卡覆盖）；tactic_category/tactic_phase 入 _debug；安全边界并入全局原则
- L0-L3：单条 history ≤800 字；知识库 3 条×**400 字**（v71 启用截断，话术块 p50=651 字核心在前 400；此前常量未用全量注入）；近详远略（**最近 8 条全文**+更早对方消息 ≤60 字×6 条）；STAGE_HINTS 按 stage 注入；组装：角色定位>先解读>时间>自洽>战术指令>画像>摘要>参考>格式约束
## 关键踩坑（务必先读）
