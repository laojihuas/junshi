...
const GLOBAL_TACTIC_PREAMBLE = `\n\n【战术指令】(本轮最高优先，先判断后回复)\n`
  ...
  + `全局原则（全程生效）：\n`
  + `- 回复字数统一 8-20 字（[v20260818 用户定稿] 至少 8 字保证内容量、最多 20 字封顶；通常 1-2 句短句，能拆两句就不合一句；超 20 字=失败必须压缩）。\n`
...
function buildSystemContent(opts: {
  ...
  if (opts.wb) {
    s += `\n\n【决策流程】(思考档生效，回复前按序完成，思考过程不输出)\n`
      ...
      + `④ 淘汰：对照【角色定位】人设（高价值自信、幽默轻调侃、不讨好不黏人）+ 字数 8-20 + 无标点 + 口语化，淘汰不符的，保留最稳最妙的一个。\n`
  ...
  if (opts.thinkingBudget && opts.thinking && opts.thinking !== 'off') {
    d += `\n\n【思考预算】（思考档生效，最高优先）\n最终回复只有 ≤20 字（统一字数区间），思考也必须克制：只做必要推理（潜台词/意图/策略判断），最多 3 步直接给结论，禁止长篇分析、禁止复述对话内容、禁止罗列选项。`;
  }
  ...
  if (opts.switchTopic) {
    d += `\n\n【切换话题】(本轮最高优先级，覆盖上面的所有目标与推进指令)\n`
      ...
      + `- 本轮任务：给出一句可以直接发给对方的新话题开场白（1 句，8-20 字，带钩子/情绪/好奇心）。\n`
