...
//
// [v83 复制/生成双轨字数]（2026-08-08，弹药保味 + 防小作文）
//   - 背景：检索弹药（含低俗/擦边金句）被"禁止照抄+≤20字"双重消毒——模型被迫
//     压缩改写，一改就丢韵味；放宽到 40 字又会变成小作文
//   - 字数双轨：自己临场生成 → 整条 ≤20字；原样采用【参考资料】里的话术（复制路径）
//     → 可达 ≤40字，只许替换人称、禁止改写润色（复制不经语义评估 → 不被消毒）
//   - 改动：GLOBAL_TACTIC_PREAMBLE 执行顺序（优先挑参考原样发出）+ 字数规则双轨；
//     【参考资料】引导语从"仅弹药"升级为"弹药库：优先原样选用"
//
// 请求体（JSON）：
...
const GLOBAL_TACTIC_PREAMBLE = `\n\n【战术指令】(本轮最高优先，先判断后回复)\n`
  + `执行顺序：①判断她这条消息属于防守/进攻/救场哪一类 ②匹配对应场景 ③按该场景的"态度+手法"输出一句话话术——优先从【参考资料】里挑 1 句最贴合当前语境的原样发出（复制路径），没有合适参考再自己临场生成。\n`
  + `全局原则（全程生效）：\n`
  + `- 字数规则（双轨）：自己临场生成的话术 → 整条 ≤20字（超20字=失败必须压缩）；原样采用【参考资料】里的话术 → 可达 ≤40字，保留原文韵味和节奏，只许替换人称，禁止改写润色。\n`
  + `- 无论生成还是复制，回复都 ≤ 她字数的1.3倍，通常一句；她回复越短越敷衍，你延迟越久（模拟高价值）；陈述句 > 提问句，用冷读代替查户口；永远不让她觉得你在"等"她消息。\n`
...
function buildSystemContent(opts: {
  ...
  if (opts.kbItems.length > 0) {
    ...
    // [v83 复制路径] 引导语从"仅弹药"升级为"优先原样选用"（复制≤40字，见战术前导字数双轨）
    s += `\n\n【参考资料】（弹药库：优先从中选 1 句最贴合当前语境的原样发出——复制路径 ≤40字，只许替换人称、不许改写；与当前对话冲突时以对话连续性为准）\n${kbText}`;
    if (opts.kbFallback) {
