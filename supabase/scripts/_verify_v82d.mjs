// v82d 验证：中性平淡话题（吃饭/日常，无情绪负担）→ 里程碑小目标织入
...
const email = `v82d_${Date.now()}@jssl.local`;
...
await fetch(`${BASE}/rest/v1/rpc/register_account`, { method: 'POST', headers: { Authorization: `Bearer ${JWT}`, apikey: JWT, 'Content-Type': 'application/json' }, body: JSON.stringify({ p_account_user_id: uid, p_account_name: 'v82d验证', p_device_id: 'v82d_dbg_000001' }) });
await fetch(`${BASE}/rest/v1/rpc/login_account`, { method: 'POST', headers: { Authorization: `Bearer ${JWT}`, apikey: JWT, 'Content-Type': 'application/json' }, body: JSON.stringify({ p_account_user_id: uid, p_session_id: 'v82d_session_001' }) });
...
const sid = await fetch(`${BASE}/rest/v1/chat_sessions`, { method: 'POST', headers: { Authorization: `Bearer ${accountToken}`, apikey: JWT, 'Content-Type': 'application/json', Prefer: 'return=representation' }, body: JSON.stringify({ user_id: uid, note: 'v82d验证', friend_name: '测试' }) });
const sessionIdDb = (await sid.json())?.[0]?.id;
const card = {
  profile: { stage: '暧昧', personality: '开朗', relationship_note: '聊了几天，氛围轻松', recent_events: '' },
  milestones: [],
  goal: '推进恋爱',
  recent_user_messages: ['你中午吃了吗', '我也还没吃'],
  recent_self_messages: ['还没呢 你吃了吗', '那你打算吃啥'],
};
...
const queries = ['中午不知道吃啥', '天天外卖 吃腻了', '今晚想吃火锅', '你们那边有什么好吃的', '改天带我去吃呗'];
const history = [];
...
for (const q of queries) {
  ...
  const r = await fetch(`${BASE}/functions/v1/ima-proxy`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${accountToken}`, apikey: JWT, 'Content-Type': 'application/json', 'X-Identity-Type': 'account', 'X-Session-Id': 'v82d_session_001' },
